...
int init(const char *filepath, void **handle)
{
	...
	strcpy_s(ph->filepath, strlen(filepath) + 1, filepath);
	...
	while (!feof(fp)) {
		...
		strcpy_s(pMalloc->value, strlen(valuebuff)+1, valuebuff);
...
int getValue(void *handle, const char *key, char *value)
{
	...
	strcpy_s(value, strlen(pCurrent->value)+1, pCurrent->value);
...
int setValue(void *handle, const char *key, const char *value)
{
	...
	// 修改key的value
	strcpy_s(pCurrent->value, strlen(value)+1, value);
	if (strchr(value, '\n') == NULL) {  // 加一个换行符
		strcat_s(pCurrent->value, VALUE_SIZE, "\n");
	}
...
// 将<key, value>用XML格式存储
int Save2Xml(const char* xmlpath, void *handle) {
	PROPS_HANDLE *ph = NULL;
	ph = (PROPS_HANDLE *)handle;
	Properties *head = ph->pHead;

	int ret = 0, writeLen = 0;
	FILE *fp = NULL;
	Properties *pCurrent = NULL;
	if (ph->filepath == NULL || head == NULL) {
		ret = -100;
		printf("fun saveConfig error:%d from (filepath == NULL || head == NULL)\n", ret);
		return ret;
	}

	fopen_s(&fp, xmlpath, "w");
	
	fprintf(fp, "<config>\n");

	pCurrent = head->pNext;
	while (pCurrent != NULL) {
		writeLen = fprintf(fp, "<%s>%s</%s>\n", pCurrent->key, pCurrent->value, pCurrent->key);    // 返回写入的字节数，出现错误返回一个负值
...
int add(void *handle, const char *key, const char *value)
{
	...
	strcpy_s(pMalloc->key, strlen(key)+1, key);
	if (strchr(pCurrent->value, '\n') == NULL) {
		strcat_s(pCurrent->value, VALUE_SIZE, "\n");
	}
	strcpy_s(pMalloc->value, strlen(value)+1, value);
	if (strchr(value, '\n') == NULL) {  // 加一个换行符
		strcat_s(pMalloc->value, VALUE_SIZE, "\n");
	}
